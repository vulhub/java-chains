...
PACKAGE_SOURCE="$ROOT/.github/scripts/package-public-from-source.sh"

python3 - "$RELEASE_WORKFLOW" "$RELEASE_MANIFEST" "$VERIFY_SOURCE" "$PACKAGE_SOURCE" <<'PY'
import re
import sys
...
package_source = open(sys.argv[4], encoding="utf-8").read()
...
if "test-verify-source-tag.sh" not in resolve:
    raise SystemExit("resolve job must run the source-tag resolver tests")
if "test-package-public-from-source-contract.sh" not in resolve:
    raise SystemExit("resolve job must run the source-packaging contract test")
...
if "CHAINS_WORKFLOW_ARTIFACTS" in verify_source or "/artifacts" in verify_source:
    raise SystemExit("source-tag resolver must not inspect Actions artifacts")
if "tag-test-build.yml" in verify_source:
    raise SystemExit("source-tag resolver must not require Tag Test and Build")
if 'ASSET_NAME="tag-build.tar.gz"' in verify_source:
    raise SystemExit("source-tag resolver must not require tag-build.tar.gz")
if "releases/tags/" in verify_source:
    raise SystemExit("source-tag resolver must not require a source GitHub Release")

if "repository: Java-Chains/chains" not in build:
    raise SystemExit("build job must check out Java-Chains/chains")
if "package-public-from-source.sh" not in build:
    raise SystemExit("build job must package from the checked-out source tag")
if "actions/setup-java" not in build:
    raise SystemExit("build job must install JDK 8 to compile the public edition")
if "actions/setup-node" not in build:
    raise SystemExit("build job must install Node to package both SPAs")
if "pnpm/action-setup" not in build:
    raise SystemExit("build job must install pnpm to package both SPAs")
if "EXPECTED_EDITION: public" not in build:
    raise SystemExit("build job must require the public edition metadata")
...
if "verify-tag-build-artifact.sh" not in build:
    raise SystemExit("build job must verify staged artifact metadata and checksums")
...
if "tag-build.tar.gz" in build:
    raise SystemExit("build job must not fetch tag-build.tar.gz from the source GitHub Release")
...
for forbidden in (
    ...
    if forbidden in build:
        raise SystemExit(f"public release workflow must not include internal bits: found {forbidden}")

if "check-public-edition-surface.sh" not in package_source or "--source-only" not in package_source:
    raise SystemExit("source packaging must run the public-edition source preflight")
if "PublicInternalFeaturesGateTest" not in package_source:
    raise SystemExit("source packaging must run the public-edition unit contract")
if "-Pfrontend" not in package_source:
    raise SystemExit("source packaging must build the public frontend profile")
...
if "base/RELEASE-NOTES.md" not in create:
    raise SystemExit("release creation must publish the generated notes")
...
if "publish_release == 'true'" not in docker_build:
...
if "tag-build.tar.gz" in manifest:
    raise SystemExit("release manifest must not document tag-build.tar.gz consumption")
if not re.search(r"(?m)^\s+ref:\s+[0-9a-f]{40}\s*$", manifest):
    raise SystemExit("chains-config manifest must pin an immutable commit")

print("PASS: public release checks out the source tag, tests the public edition, and packages it")
PY
